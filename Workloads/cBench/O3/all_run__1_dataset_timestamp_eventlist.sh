...
if (( $# != 2 )); then
  echo "This program requires CPU Frequency in MHz input and integer header flag (0 means no header)." >&2
  exit 1
fi

(( $2 )) && echo -e "#Name\tDataset\tCPU Frequency(MHz)\tStart(ns)\tEnd(ns)\tCycles"
