...
do
    ...
    if (( "$core_select"_CHOSEN )); then
	    ...
	    do	
		#This is needed to switch successfully to LITTLE. This problem occurs when we want to use 500Mhz on LITTLE. Since this frequency is both present we need to switch to a lower level then bring it up to 500Mhz
		if [[ $core_select == "LITTLE" ]]; then
			cpufreq-set -f "175Mhz"
		fi
		...
		echo "Core frequency: $freq_select""Mhz"
            ...
            if (( $SAVE_DIR_CHOSEN )); then
                ...
                cp -v "benchmarks_data_$core_select.dat" "$save_dir/Run_$i"
                rm -v "benchmarks_data_$core_select.dat"
            else
                echo "Copying results to dir: Run_$i"
                mkdir -v "Run_$i"
                cp -v "benchmarks_data_$core_select.dat" "Run_$i" 
                rm -v "benchmarks_data_$core_select.dat"
            fi
