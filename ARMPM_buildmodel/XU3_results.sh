...
do
    ...
    if (( "$core_select"_CHOSEN )); then
		...
		#CPU Power(W)\tCPU Voltage(V)\tCPU Frequency(MHz)\tCPU Temperature(C)\tCycles\tL1 DCache Access\tL1 ICache Access\tInstructions\tRAM Access\tCPU User(%)\tCPU Sys(%)\tCPU Idle(%)\tCPU I/O Wait(%)\tCPU IRQ(%)\tCPU Soft IQ(%)"
